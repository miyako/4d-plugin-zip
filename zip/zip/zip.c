 ...
 */
extern int ZEXPORT zipOpenNewFileInZip4_64_noAES (zipFile file, const char* filename, const zip_fileinfo* zipfi,
...
extern int ZEXPORT zipOpenNewFileInZip3_64_noAES(zipFile file, const char* filename, const zip_fileinfo* zipfi,
...
{
    return zipOpenNewFileInZip4_64_noAES(file, filename, zipfi, extrafield_local, size_extrafield_local,
...
extern int ZEXPORT zipOpenNewFileInZip64_noAES(zipFile file, const char* filename, const zip_fileinfo* zipfi,
    const void* extrafield_local, uInt size_extrafield_local, const void*extrafield_global,
...
local int zip64FlushWriteBuffer_noAES(zip64_internal* zi)
{
    int err=ZIP_OK;
...
extern int ZEXPORT zipWriteInFileInZip_noAES (zipFile file,const void* buf,unsigned int len)
{
    zip64_internal* zi;
    int err=ZIP_OK;
    ...
    if(zi->ci.method == Z_BZIP2ED && (!zi->ci.raw))
    {
      ...
      while ((err==BZ_RUN_OK) && (zi->ci.bstream.avail_in>0))
      {
        if (zi->ci.bstream.avail_out == 0)
        {
          if (zip64FlushWriteBuffer_noAES(zi) == ZIP_ERRNO)
            err = ZIP_ERRNO;
          zi->ci.bstream.avail_out = (uInt)Z_BUFSIZE;
          zi->ci.bstream.next_out = (char*)zi->ci.buffered_data;
        }
...
extern int ZEXPORT zipCloseFileInZipRaw_noAES(zipFile file, uLong uncompressed_size, uLong crc32)
{
    return zipCloseFileInZipRaw64_noAES (file, uncompressed_size, crc32);
}
...
extern int ZEXPORT zipCloseFileInZipRaw64_noAES (zipFile file, ZPOS64_T uncompressed_size, uLong crc32)
{
    zip64_internal* zi;
    ZPOS64_T compressed_size;
    uLong invalidValue = 0xffffffff;
...
extern int ZEXPORT zipCloseFileInZip_noAES(zipFile file)
{
    return zipCloseFileInZipRaw_noAES(file, 0, 0);
}
